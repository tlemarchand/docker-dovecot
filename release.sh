...
podman run -e 'DEBIAN_FRONTEND=noninteractive' -v ./dovecot.sources:/tmp/dovecot.sources:Z --name dovecot debian:bullseye-slim /bin/bash -c "apt-get update > /dev/null && apt-get install -y curl apt-transport-https gpg > /dev/null && \
